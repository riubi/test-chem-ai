...
**Лучший результат: LB 274.085** (файл `submission_best_lb274.csv`) — получен серией скриптов с псевдо-разметкой за пределами ноутбука; в текущей версии ноутбука не воспроизводится.

**Ноутбук воспроизводит: LB ~278** — простой ансамбль из 4 моделей (ExtraTrees + LightGBM + XGBoost + CatBoost), равные веса, log1p только для SI. Секция 16c.

Расширенный бленд (log1p + rank-Gauss + seed-bag + SLSQP per-target, секции 11-16b) даёт OOF mean ~498.7, но на public LB проигрывает простому ансамблю на 16 пунктов (LB 294.67 против 278). Причина: SLSQP по 55 кандидатам переобучается на OOF, а log/rank-Gauss модели систематически занижают IC50/CC50 в верхних квантилях (см. `decisions.md` §0).
